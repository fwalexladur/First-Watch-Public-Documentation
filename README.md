# First Watch Public Documentation
First Watch Public Documentation

This repository contains **public-facing documentation** for the First Watch® Platform.

It is intended for:
- Customers
- Partners
- Auditors
- Pre-sales and technical stakeholders

The documentation includes:
- User Manual
- Technical Briefs
- Public Release Notes

⚠️ Deployment guides, internal architecture, and engineering documentation
are maintained in a **separate restricted repository**.


# First Watch® Platform – Documentation Index

Welcome to the public documentation for the First Watch® Platform.

## Documentation Sets

📘 **User Manual**  
Operational guidance for daily users and administrators.  
➡️ [Open User Manual](docs/user-manual/README.md)


🧠 **Technical Briefs**  
High-level technical and architectural documentation for technical stakeholders.  
➡️ [Open Technical Briefs](./technical-briefs/README.md)

🗂 **Release Notes**  
Version history, improvements, and security updates.  
➡️ [Open Release Notes](./release-notes/README.md)
